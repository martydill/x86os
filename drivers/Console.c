...
STATUS ConsoleDeviceWrite(char* buffer, int numBytes) { return S_OK; }
...
STATUS ConActivateConsole(BYTE number) {
  ...
  // DWORD offset = jvj
  DWORD offset = CONSOLE_SIZE * activeConsole /
                 2; // Divide by 2 because the low word is only 7 bits
  Debug("Switch to console %d at %d %d %d %d\n", number, offset, pActiveConsole,
...
STATUS ConpUpdateHardwareCursor(void) {
  WORD position = pActiveConsole->cursorY * CONSOLE_WIDTH +
                  pActiveConsole->cursorX + offset;
  // Debug("%d %d %d\n", pActiveConsole->cursorY, pActiveConsole->cursorX,
  // position);
...
void ScrollDown() {
  Memcopy(pActiveConsole->buffer, pActiveConsole->buffer + (CONSOLE_WIDTH * 2),
          (CONSOLE_WIDTH * (CONSOLE_HEIGHT - 1) * 2));
  Memset(pActiveConsole->buffer + (CONSOLE_WIDTH * 2) * (CONSOLE_HEIGHT - 1), 0,
         CONSOLE_WIDTH * 2);
...
STATUS ConBackspace(void) {
  ...
  pActiveConsole->buffer[(pActiveConsole->cursorY * CONSOLE_WIDTH +
                          pActiveConsole->cursorX) *
                         2] = ' ';
...
STATUS ConDisplayString(const char* str, WORD x, WORD y) {
  ...
  // separately newX = (loc % (CONSOLE_WIDTH * 2)) / 2; newY = loc /
  // (CONSOLE_WIDTH * 2);
  ConMoveCursor(point.X, point.Y);
...
STATUS ConDestroy(void) { return S_OK; }
