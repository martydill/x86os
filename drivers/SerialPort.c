...
int SerialPortIsBusy()
{
    return (IoReadPortByte(COM1_PORT + 5) & 0x20) == 0;
}
...
STATUS SerialPortInit(void)
{
    ...
    IoWritePortByte(COM1_PORT + REGISTER_INTERRUPT_ENABLE, 0x01);       // HI (0c => 9600 baud)

    IoWritePortByte(COM1_PORT + REGISTER_LINE_CONTROL, 0x03);       // clear DLAB & set 8-N-1
