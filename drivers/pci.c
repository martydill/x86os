...
DWORD PciReadConfigByte(BYTE bus, BYTE device, BYTE function, BYTE reg)
{
  ...
  DWORD rawAddress = busAddress | deviceAddress | functionAddress | 0x80000000 | (reg << 2);
...
char* PciGetDevice(WORD vendorId, WORD deviceId) {
  switch(vendorId) {
    ...
    case 0x1022:
      switch(deviceId) {
        case 0x2000: return "79c970 [PCnet32 LANCE]";
        default: return "Unknown";
      }
...
STATUS PciInit(void)
{
    ...
    for(BYTE bus = 0; bus < 0xFF; ++bus)
    {
      for(BYTE device = 0; device < 0x16; ++device)
      {
        ...
        if(dev.vendorId == PCI_INVALID_VENDOR) continue;

        KPrint("%s %d, %s\n", PciGetVendor(dev.vendorId), dev.deviceId, PciGetDevice(dev.vendorId, dev.deviceId), PciGetClass(dev.classCode));
      }
    }
    return S_OK;
}
