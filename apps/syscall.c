...
void KPrint(const char* data) {
  _syscall(SYSCALL_KPRINT, (unsigned int)data, 0, 0, 0, 0);
}

void _exit(int code) { _syscall(SYSCALL_EXIT, (unsigned int)code, 0, 0, 0, 0); }

// TODO handle differences between exit and _exit
void exit(int code) { _syscall(SYSCALL_EXIT, (unsigned int)code, 0, 0, 0, 0); }

void Mount(const char* mountPoint, const char* destination) {
  _syscall(SYSCALL_MOUNT, (unsigned int)mountPoint, (unsigned int)destination,
           0, 0, 0);
}

int open(const char* pathname, int flags) {
  return _syscall(SYSCALL_OPEN, (unsigned int)pathname, (unsigned int)flags, 0,
                  0, 0);
}

int read(int fd, void* buf, int count) {
  return _syscall(SYSCALL_READ, (unsigned int)fd, (unsigned int)buf,
                  (unsigned int)count, 0, 0);
} // TODO ssize_t, size_t

int write(int fd, void* buf, int count) {
  return _syscall(SYSCALL_WRITE, (unsigned int)fd, (unsigned int)buf,
                  (unsigned int)count, 0, 0);
} //
...
int posix_spawn(pid_t* restrict pid, const char* restrict path,
  ...
  return _syscall(SYSCALL_POSIX_SPAWN, (unsigned int)pid, (unsigned int)path,
                  (unsigned int)attrp, (unsigned int)argv, (unsigned int)envp);
}

pid_t waitpid(pid_t pid, int* status_ptr, int options) {
  return _syscall(SYSCALL_WAITPID, (unsigned int)pid, (unsigned int)status_ptr,
...
int closedir(DIR* dir) {
  return _syscall(SYSCALL_CLOSEDIR, (unsigned int)dir, 0, 0, 0, 0);
}
...
char* getcwd(char* buf, size_t size) {
  return (char*)_syscall(SYSCALL_GETCWD, (unsigned int)buf, (unsigned int)size,
                         0, 0, 0);
}
...
int fstat(int fildes, struct stat* buf) {
  return _syscall(SYSCALL_FSTAT, (unsigned int)fildes, (unsigned int)buf, 0, 0,
                  0);
}